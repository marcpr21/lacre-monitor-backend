...
PHOTOS = []
AUTHORIZATIONS = {}
EMAIL_CONFIG = {'admin_email': '', 'enabled': False, 'alerts': {}}

def init_users():
    admin_pw = bcrypt.hashpw(b'admin123', bcrypt.gensalt())
    USERS['admin'] = {'id': str(uuid.uuid4()), 'username': 'admin', 'password': admin_pw, 'name': 'Administrador', 'role': 'admin'}
    ...
    for name in employee_names:
        emp_pw = bcrypt.hashpw(b'123456', bcrypt.gensalt())
        username = name.lower().replace(' ', '_')
        USERS[username] = {'id': str(uuid.uuid4()), 'username': username, 'password': emp_pw, 'name': name, 'role': 'employee'}
    teste_pw = bcrypt.hashpw(b'teste', bcrypt.gensalt())
    USERS['teste'] = {'id': str(uuid.uuid4()), 'username': 'teste', 'password': teste_pw, 'name': 'Usuário Teste', 'role': 'employee'}
...
def get_employees(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    return [UserResponse(id=u['id'], username=u['username'], name=u['name'], role=u['role']) for u in USERS.values() if u['role'] == 'employee']
...
def authorize_photo(auth: Authorization, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if payload['role'] != 'admin':
        raise HTTPException(status_code=403, detail='Acesso negado')
    if auth.employee_id not in AUTHORIZATIONS:
        AUTHORIZATIONS[auth.employee_id] = {}
    expires_at = get_brazil_time() + timedelta(hours=auth.duration_hours)
    AUTHORIZATIONS[auth.employee_id][auth.photo_type] = {'authorized': True, 'expires_at': expires_at.isoformat(), 'authorized_by': payload['username']}
